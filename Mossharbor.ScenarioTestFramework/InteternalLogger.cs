    ...
    internal class InternalLogger
    {
        ...
        private string GenerateScreenShot(string screenShotFileName)
        {
            try
            {
                ...
                if (screensaver)
                {
                    InternalLogger.Instance.LogSubResult(Result.InfrastructureWarning, "Screensaver is currently enabled (should be disabled for testing)");
                    ...
                    if (screensaver)
                    {
                        ...
                        InternalLogger.Instance.LogSubResult(Result.InfrastructureWarning, "Screensaver is currently running");
                    }
                }
                ...
                finally
                {
                    ...
                    {
                        InternalLogger.Instance.LogSubResult(Result.InfrastructureWarning, "Could not access window input handle, machine is probably locked");
                    }
                }

                // Make sure the directory exists
                if (!Directory.Exists(InternalLogger.Instance.ScreenShotDirectory))
                {
                    Directory.CreateDirectory(InternalLogger.Instance.ScreenShotDirectory);
                }

                // Get the screen shot
                string screenShotFilenameAndPath = ScreenCapture.CaptureScreenshotWithFileName(InternalLogger.Instance.ScreenShotDirectory, screenShotFileName);

                if (String.IsNullOrEmpty(screenShotFilenameAndPath))
                {
                    InternalLogger.Instance.LogSubResult(Result.InfrastructureWarning, null, "Couldn't create screenshot!");
                    return null;
                }
            ...
            {
                InternalLogger.Instance.LogSubResult(Result.InfrastructureWarning, e, "Couldn't create screenshot!");
                return null;
            }
        }
        ...
        private class SingletonLoggerInstance
        {
            ...
            /// singleton object.
            /// </summary>
            internal static readonly InternalLogger Instance = new InternalLogger();
        }
    }
}
